...
function howManyMovies(moviesArr) {
  let filterMovies = moviesArr.filter((movie) => {
...
function scoresAverage(moviesArr) {
  let avgScores = moviesArr.reduce((acc, movie) => {
  ...
  let avg = avgScores / moviesArr.length  
...
function dramaMoviesScore(moviesArr) {
  const filterDramaMovies = moviesArr.filter((movie) => {
...
function orderByYear(moviesArr) {
  let moviesArrCloned = JSON.parse(JSON.stringify(moviesArr)); // deep clone to not mutate original
  let sortByYear = moviesArrCloned.sort((a, b) => {
    if (a.year > b.year){
      return 1
...
function orderAlphabetically(moviesArr) {
  let moviesArrCloned = JSON.parse(JSON.stringify(moviesArr)); // deep clone to not mutate original
  let sortAbc = moviesArrCloned.sort((a, b) => {
    if (a.title > b.title){
      return 1
